...
import com.hengxuan.ehealthplatform.lens.iris.MultiTouchController;
...
public class SkinTouchView extends ImageView implements
	...
	public class Img {
		...
		/*
		 * private boolean setPos(float cx, float cy, float sx, float sy) {
		 * if((cx+displayWidth/2)<width*sx && (cx-displayWidth/2)>0) { minX =
		 * displayWidth/2 -cx; maxX = minX +width*sx; centerX = minX + (maxX -
		 * minX) / 2F; } else { if((cx+displayWidth/2)>width*sx) { maxX =
		 * displayWidth; minX = maxX - width*sx; centerX = minX + (maxX - minX)
		 * / 2F; } else { minX = 0; maxX = minX +width*sx; centerX = minX +
		 * (maxX - minX) / 2F; } } if((cy+displayHeight/2)<height*sy &&
		 * (cy-displayHeight/2)>0) { minY = displayHeight/2 -cy; maxY =
		 * minY+height*sy; centerY = minY + (maxY - minY) / 2F; } else {
		 * if((cy+displayHeight/2)>height*sy) { maxY = displayHeight; minY =
		 * maxY - height*sy; centerY = minY + (maxY - minY) / 2F; } else { minY
		 * = 0; maxY = minY + height*sy; centerY = minY + (maxY - minY) / 2F; }
		 * } scaleX = sx; scaleY = sy; return true; }
		 */
			...
			if (drawLine) {
				...
				case SkinOtherAnalysisActivity.ANALYSIS_MODE_PORE:
					while (iter.hasNext()) {
						...
						canvas.drawCircle(pointss[0].x, pointss[0].y, 10.0f,
								paint);
						canvas.drawLine(pointss[0].x, pointss[0].y,
								pointss[1].x, pointss[1].y, paint);
						canvas.drawCircle(pointss[1].x, pointss[1].y, 10.0f,
								paint);
								...
								String.format(
										...
										PIXEL_TO_MM
												* 1440
												/ (img.maxX - img.minX)
												* (int) (Math
														.sqrt((pointss[1].x - pointss[0].x)
																* (pointss[1].x - pointss[0].x)
																+ (pointss[1].y - pointss[0].y)
																* (pointss[1].y - pointss[0].y)) / 2)),
								pointss[0].x - 20, pointss[0].y - 20, paint);
					}
					if (drawStart) {
						canvas.drawLine(startPoint.x - 10, startPoint.y,
								startPoint.x + 10, startPoint.y, paint);
						canvas.drawLine(startPoint.x, startPoint.y - 15,
								startPoint.x, startPoint.y + 15, paint);
					} else {
						paint.setColor(Color.rgb(0, 255, 0));
						canvas.drawCircle(startPoint.x, startPoint.y, 10.0f,
								paint);
								...
								String.format(
										...
										PIXEL_TO_MM
												* 1440
												/ (img.maxX - img.minX)
												* (int) (Math
														.sqrt((endPoint.x - startPoint.x)
																* (endPoint.x - startPoint.x)
																+ (endPoint.y - startPoint.y)
																* (endPoint.y - startPoint.y)) / 2)),
								startPoint.x - 20, startPoint.y - 20, paint);
					}
					break;
				case SkinOtherAnalysisActivity.ANALYSIS_MODE_ACNE:
					while (iter.hasNext()) {
						...
						canvas.drawCircle(
								(pointss[0].x + pointss[1].x) / 2,
								(pointss[0].y + pointss[1].y) / 2,
								(int) Math.sqrt((pointss[1].x - pointss[0].x)
										* (pointss[1].x - pointss[0].x)
										+ (pointss[1].y - pointss[0].y)
										* (pointss[1].y - pointss[0].y)) / 2,
								paint);
										...
										PIXEL_TO_MM
												...
												* (int) (Math
														.sqrt((pointss[1].x - pointss[0].x)
																...
																* (pointss[1].y - pointss[0].y)) / 2)),
								(pointss[0].x + pointss[1].x) / 2,
								(pointss[0].y + pointss[1].y) / 2, paint);
					}
					if (drawStart) {
						canvas.drawLine(startPoint.x - 10, startPoint.y,
								startPoint.x + 10, startPoint.y, paint);
						...
						canvas.drawCircle(
								...
								(int) Math.sqrt((endPoint.x - startPoint.x)
										* (endPoint.x - startPoint.x)
										+ (endPoint.y - startPoint.y)
										* (endPoint.y - startPoint.y)) / 2,
								paint);
										...
										PIXEL_TO_MM
												...
												* (int) (Math
														.sqrt((endPoint.x - startPoint.x)
																...
																* (endPoint.y - startPoint.y)) / 2)),
								(startPoint.x + endPoint.x) / 2,
								(startPoint.y + endPoint.y) / 2, paint);
					}
					break;
				case SkinOtherAnalysisActivity.ANALYSIS_MODE_SENSI_NUM:
					while (iter.hasNext()) {
						PointF[] pointss = (PointF[]) iter.next();
						paint.setColor(Color.rgb(255, 0, 0));
						canvas.drawText("1", pointss[1].x, pointss[1].y, paint);
					}
					paint.setColor(Color.rgb(255, 0, 0));
					canvas.drawText("1", endPoint.x, endPoint.y, paint);
					break;
	...
	public void setDrawStart(boolean start) {
		drawStart = start;
		startPoint.set(((Activity) getContext()).getWindowManager()
				.getDefaultDisplay().getWidth() / 2, ((Activity) getContext())
	...
	public void setDrawLine(boolean draw) {
		...
		case SkinOtherAnalysisActivity.ANALYSIS_MODE_PORE:
			...
			break;
		case SkinOtherAnalysisActivity.ANALYSIS_MODE_ACNE:
			textView.setText(getResources().getString(R.string.acne_num) + "\n"
					+ getResources().getString(R.string.mean_radius));
			break;
		case SkinOtherAnalysisActivity.ANALYSIS_MODE_SENSI_NUM:
			textView.setText(getResources()
					.getString(R.string.blood_streak_num)
					+ "\n"
					+ getResources().getString(R.string.blood_streak_area));
			break;
		case SkinOtherAnalysisActivity.ANALYSIS_MODE_SENSI_AREA:
			textView.setText(getResources()
					.getString(R.string.blood_streak_num)
	...
	private float acneRadius;
	private int bloodStreakNum;
	private float bloodStreakArea;

	public float getPoreRadius() {
		return poreRadius;
	}

	public float getAcneRadius() {
		return acneRadius;
	}

	public int getBloodStreakNum() {
		return bloodStreakNum;
	}

	public float getBloodStreakArea() {
		return bloodStreakArea;
	}
	...
	public void setData() {
		...
		case SkinOtherAnalysisActivity.ANALYSIS_MODE_PORE:
			...
			textView.setText(getResources().getString(R.string.mean_radius)
					+ String.format("%.2f", poreRadius) + "mm");
			break;
		case SkinOtherAnalysisActivity.ANALYSIS_MODE_ACNE:
			while (iter.hasNext()) {
				PointF[] pointss = (PointF[]) iter.next();
			...
			acneRadius = radiusSum / pointList.size();
			textView.setText(getResources().getString(R.string.acne_num)
					+ pointList.size() + "\n"
					+ getResources().getString(R.string.mean_radius)
					+ String.format("%.2f", acneRadius) + "mm");
			break;
		case SkinOtherAnalysisActivity.ANALYSIS_MODE_SENSI_NUM:
			bloodStreakNum = pointList.size();
			textView.setText(getResources()
					.getString(R.string.blood_streak_num)
					+ bloodStreakNum
					+ "\n"
		...
		case SkinOtherAnalysisActivity.ANALYSIS_MODE_SENSI_AREA:
			...
			bloodStreakArea = radiusSum;
			textView.setText(getResources()
					.getString(R.string.blood_streak_num)
					+ "\n"
					+ getResources().getString(R.string.blood_streak_area)
					+ String.format("%.2f", bloodStreakArea) + "mm^2");
			break;
		default:
			break;
		}
	...
	public SkinTouchView(Context context) {
		this(context, null);
	}

	public SkinTouchView(Context context, AttributeSet attributeset) {
		this(context, attributeset, 0);
	}

	public SkinTouchView(Context context, AttributeSet attributeset, int i) {
	...
	/*
	 * @Override public volatile Object getDraggableObjectAtPoint(PointInfo
	 * pointinfo) { return getDraggableObjectAtPoint(pointinfo); }
	 */
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			if (drawStart) {
				startPoint.set(startPoint3.x + event.getX() - startPoint2.x,
						startPoint3.y + event.getY() - startPoint2.y);
			} else {
				if (analysisMode == SkinOtherAnalysisActivity.ANALYSIS_MODE_SENSI_NUM)
					endPoint.set(event.getX(), event.getY());
				else
					endPoint.set(startPoint.x + event.getX() - startPoint2.x,
							startPoint.y + event.getY() - startPoint2.y);
			}
			postInvalidate();
			break;
		case MotionEvent.ACTION_UP:
			...
			if (drawStart) {
				...
				dialog.setMessage(getResources().getString(
						R.string.whether_start_point));
				dialog.setButton(AlertDialog.BUTTON_POSITIVE, getResources()
				...
				dialog.setButton(AlertDialog.BUTTON_NEGATIVE, getResources()
				...
				dialog.setMessage(getResources().getString(
						R.string.whether_continue_mark));
				dialog.setButton(AlertDialog.BUTTON_POSITIVE, getResources()
						.getString(R.string.yes),
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
								...
								// TODO Auto-generated method stub
								showDialog = false;
								startPoint.set(((Activity) getContext())
